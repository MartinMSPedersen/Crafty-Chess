...
********************************************************************************
*                                                                              *
*   GiveCheck() is used to determine if a move checks the opponent.  this is   *
...
*                                                                              *
*   phase(1) simply determines if the piece attacks the enemy king after the   *
*   move is "made" (but without really calling MakeMove()).  for knights and   *
*   pawns, the piece must directly attack the square the king is on.  for      *
*   sliding pieces, we only check the ray connecting the destination square    *
*   with the king's square (if any.)                                           *
*                                                                              *
...
********************************************************************************
*/
int GiveCheck(int ply, int wtm, int *move)
{
  register int from, to, king_square, piece;
  register BITBOARD target, king_mask, occupied, temp, from_attacks;
/*
 ----------------------------------------------------------
|                                                          |
...
*/
  ...
  if (wtm) {
    ...
    king_mask=BlackKing(ply);
  }
  else {
    king_square=WhiteKingSQ(ply);
    king_mask=WhiteKing(ply);
  }
  piece=Promote(*move) ? Promote(*move) : Piece(*move);
  occupied=Or(WhitePieces(ply),BlackPieces(ply));
/*
 ----------------------------------------------------------
|                                                          |
...
*/
  case bishop:
    ...
    if (king_square == FirstOne(And(temp,mask_plus7dir[to]))) return(1);
    if (king_square == FirstOne(And(temp,mask_plus9dir[to]))) return(1);
    if (king_square == LastOne(And(temp,mask_minus7dir[to]))) return(1);
    if (king_square == LastOne(And(temp,mask_minus9dir[to]))) return(1);
    break;
/*
 ------------------------------------------------
|                                                |
...
*/
  case rook:
    ...
    if (king_square == FirstOne(And(temp,mask_plus1dir[to]))) return(1);
    if (king_square == FirstOne(And(temp,mask_plus8dir[to]))) return(1);
    if (king_square == LastOne(And(temp,mask_minus1dir[to]))) return(1);
    if (king_square == LastOne(And(temp,mask_minus8dir[to]))) return(1);
    break;
/*
 ------------------------------------------------
|                                                |
...
|                                                          |
 ----------------------------------------------------------
*/
  if (!directions[king_square][from] || (Piece(*move) == queen)) return(0);
  if (wtm) from_attacks=And(AttacksTo(from,ply),WhitePieces(ply));
  else from_attacks=And(AttacksTo(from,ply),BlackPieces(ply));

  switch (directions[king_square][from]) {
    case +1:
      if (Piece(*move) == rook) return(0);
      if (LastOne(And(mask_minus1dir[from],occupied))
          != king_square) return(0);
      target=And(mask_plus1dir[from],from_attacks);
      break;
    case +7:
      if (Piece(*move) == bishop) return(0);
      if (LastOne(And(mask_minus7dir[from],occupied))
          != king_square) return(0);
      target=And(mask_plus7dir[from],from_attacks);
      break;
    case +8:
      if (Piece(*move) == rook) return(0);
      if (LastOne(And(mask_minus8dir[from],occupied))
          != king_square) return(0);
      target=And(mask_plus8dir[from],from_attacks);
      break;
    case +9:
      if (Piece(*move) == bishop) return(0);
      if (LastOne(And(mask_minus9dir[from],occupied))
          != king_square) return(0);
      target=And(mask_plus9dir[from],from_attacks);
      break;
    case -1:
      if (Piece(*move) == rook) return(0);
      if (FirstOne(And(mask_plus1dir[from],occupied))
          != king_square) return(0);
      target=And(mask_minus1dir[from],from_attacks);
      break;
    case -7:
      if (Piece(*move) == bishop) return(0);
      if (FirstOne(And(mask_plus7dir[from],occupied))
          != king_square) return(0);
      target=And(mask_minus7dir[from],from_attacks);
      break;
    case -8:
      if (Piece(*move) == rook) return(0);
      if (FirstOne(And(mask_plus8dir[from],occupied))
          != king_square) return(0);
      target=And(mask_minus8dir[from],from_attacks);
      break;
    case -9:
      if (Piece(*move) == bishop) return(0);
      if (FirstOne(And(mask_plus9dir[from],occupied))
          != king_square) return(0);
