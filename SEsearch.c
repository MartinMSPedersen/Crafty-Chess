...
int Search(TREE *tree, int alpha, int beta, int wtm, int depth,
  ...
  int threat=0, singular=1, ovalue, oovalue, bestmove=0;
...
*/
	...
	bestmove = tree->current_move[ply];
      }
      else {
        extensions=Min(extensions,0);
	...
	if (singular)
	  {
	    if (depth+extensions >= INCPLY)
	      value=-Search(tree,-alpha+SINGTHRESH,-alpha+SINGTHRESH+1,ChangeSide(wtm),
			    depth+extensions,ply+1,DO_NULL);
	    else
	      value=-Quiesce(tree,-alpha+SINGTHRESH,-alpha+SINGTHRESH+1,ChangeSide(wtm),ply+1);
	    ...
	    if (value < (alpha-SINGTHRESH))
	      {
		/* still below the singular window,all ok */
	      }
	    else
	      {
		/* old move cannot be singular, but this one can */
		/* research it with the original window */
		...
		if (value > alpha)
		  {
		    /* new best move, check whether it is singular */
		    /* we need to 'up' beta to get the real score distance 
		       between both moves */
		   if (depth+extensions >= INCPLY)
		     value=-Search(tree,-beta-SINGTHRESH,-value,ChangeSide(wtm),
				   depth+extensions,ply+1,DO_NULL);
		   else
		     value=-Quiesce(tree,-beta-SINGTHRESH,-value,ChangeSide(wtm),ply+1);
		
		    /* decide whether this new best move can still be singular  */
		   if ((value - alpha) >= SINGTHRESH)
		     {
		       /* it is a lot better , so it still can be singular */
		       /* it might cause a beta cutoff now(?), so be carefull in 
			  the fh-singular extensions */
		       /* me must also remember it for research */
	               bestmove = tree->current_move[ply];
		     }
		   else
		     {
		       /* not enough better, not singular */
		       singular = 0;
		     }
		  }
		else
		  {
		    /* not a new best move, not singular */
		    singular = 0;
		  }
	      }
	}
	...
	{
	  /* we already know there can be no singular extensions
	     at this node */
	  if (depth+extensions >= INCPLY)
	    value=-Search(tree,-alpha-1,-alpha,ChangeSide(wtm),
			  depth+extensions,ply+1,DO_NULL);
	...
	if (abort_search || tree->stop) 
	  ...
	  if (singular)
	    {
	      /* FH-Singular code */
	      /* beta cutoff...verify singularity first */
	      while ((tree->phase[ply]=(tree->in_check[ply]) ?
		      NextEvasion(tree,ply,wtm) : NextMove(tree,ply,wtm))) 
		{
                  tree->extended_reason[ply]&=check_extension;
	          /* Reduce the depth to prevent degrading to minimax  */
		  extensions=-60+REDUC;
		  
		  MakeMove(tree,ply,tree->current_move[ply],wtm);
		  if (tree->in_check[ply] || !Check(wtm)) 
		    {
		      /* checking moves are extended here */
		      if (Check(ChangeSide(wtm))) 
			{
			  tree->in_check[ply+1]=1;
			  tree->extended_reason[ply+1]=check_extension;
		      ...
		      else 
			{
		      ...
		      /* check whether they are singularly worse than the best */
		      if (depth+extensions >= INCPLY)
			value=-Search(tree,-ovalue+SINGTHRESH,-ovalue+SINGTHRESH+1,
				      ChangeSide(wtm), depth+extensions,ply+1,NO_NULL);
		      else
			value=-Quiesce(tree,-ovalue+SINGTHRESH,-ovalue+SINGTHRESH+1,
				       ChangeSide(wtm),ply+1);
		      
		      
		      if (value >= (ovalue-SINGTHRESH))
			{
			  /* the old one is not singular, but maybe the new one is*/
			  
			  /* research to check whether it is better */
			  if (depth+extensions-REDUC >= INCPLY)
			    value=-Search(tree,-ovalue-1,-ovalue, ChangeSide(wtm), 
					  depth+extensions-REDUC,ply+1,DO_NULL);
			  else
			    value=-Quiesce(tree,-ovalue-1, -ovalue, ChangeSide(wtm),ply+1);
			  
			  if (value > ovalue)
			    {
			      oovalue = ovalue;
			      /* it is better, now check if it singularly better */
			      if (depth+extensions-REDUC >= INCPLY)
				value=-Search(tree,-oovalue-SINGTHRESH,-oovalue, ChangeSide(wtm), 
					      depth+extensions-REDUC,ply+1,DO_NULL);
			      else
				value=-Quiesce(tree,-oovalue-SINGTHRESH, -oovalue, ChangeSide(wtm),ply+1);
			      ovalue = value;
			  
			      if ((value - oovalue) >= SINGTHRESH)
				{
				  /* better, even singularly better */
				  /* store it as new best */
				  oovalue = value;
				  bestmove = tree->current_move[ply];
				} 
			      else
				{
				  /* not singularly better, so no singularity */
				  singular = 0;
				  UnMakeMove(tree,ply,tree->current_move[ply],wtm);
				  break;
				}
		 ...
		 UnMakeMove(tree,ply,tree->current_move[ply],wtm);
	      ...
	      if (singular)
	      {
		  /* the move was singular, so research it now */ 
		  
		  extensions=-60+((full_extension)?singular_depth:singular_depth>>1);
		  
		  MakeMove(tree,ply,bestmove,wtm);
		  if (Check(ChangeSide(wtm))) 
		  {
		       tree->in_check[ply+1]=1;
		       extensions += 60;
		  ...
		  if (depth+extensions >= INCPLY)
	             oovalue=-Search(tree,-beta,-alpha, ChangeSide(wtm),
			    depth+extensions,ply+1,DO_NULL);
	          else
	             oovalue=-Quiesce(tree,-beta,-alpha,ChangeSide(wtm),ply+1);
		  UnMakeMove(tree,ply,bestmove,wtm);
	      }
	  }
	  
	  value = oovalue;

	  tree->current_move[ply] = bestmove;
	  HashStore(tree,ply,depth,wtm,LOWER,value,threat);
  ...
  if (moves_searched == 0) {
    ...
    if (singular) 
    {
		  ...
		  else 
		  {
	          ...
	          if (depth+extensions >= INCPLY)
	             alpha=-Search(tree,-beta,-alpha, ChangeSide(wtm),
			    depth+extensions,ply+1,DO_NULL);
	          else
	             alpha=-Quiesce(tree,-beta,-alpha,ChangeSide(wtm),ply+1);
