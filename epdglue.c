...
static
int
EGIterate(siT wtm_flag, siT think_flag)
{
int value;
int move;
mptrT mptr;
...
static
epdptrT
EGCommHandler(epdptrT epdptr0, siptrT flagptr)
{
epdptrT epdptr1;
eopptrT eopptr;
charptrT s;
	...
	while (flag && (fgets(ev, (epdL - 1), fptr0) != NULL))
		...
		if (epdptr == NULL)
			...
			if (!EPDIsLegal())
				...
				result =
					EGIterate(EGMapToHostColor(EPDFetchACTC()), think);
				...
				if ((eopptr != NULL) &&
					...
					if (eopptr != NULL)
						...
						if (eovptr == NULL)
							...
							sprintf(tbufv, "PFLC: record %ld:   cook: %s",
								(record + 1), s);
							EGPrintTB();
		...
		if (bms.bms_total != 0)
			...
			if (bms.bms_acdflag)
				...
				sprintf(tbufv, "total acd: %ld   mean total acd: %.2f\n",
					bms.bms_total_acd,
					((lrT) bms.bms_total_acd / bms.bms_total));
				EGPrintTB();
				};
			...
			if (bms.bms_acnflag)
				...
				sprintf(tbufv, "total acn: %ld   mean total acn: %.2f\n",
					bms.bms_total_acn,
					((lrT) bms.bms_total_acn / bms.bms_total));
				EGPrintTB();
				};
			...
			if (bms.bms_acsflag)
				...
				sprintf(tbufv, "total acs: %ld   mean total acs: %.2f\n",
					bms.bms_total_acs,
					((lrT) bms.bms_total_acs / bms.bms_total));
				EGPrintTB();
				};
			...
			if (bms.bms_solve != 0)
				...
				if (bms.bms_acdflag)
					...
					sprintf(tbufv, "solve acd: %ld   mean solve acd: %.2f\n",
						bms.bms_solve_acd,
						((lrT) bms.bms_solve_acd / bms.bms_solve));
					EGPrintTB();
					};
				...
				if (bms.bms_acnflag)
					...
					sprintf(tbufv, "solve acn: %ld   mean solve acn: %.2f\n",
						bms.bms_solve_acn,
						((lrT) bms.bms_solve_acn / bms.bms_solve));
					EGPrintTB();
					};
				...
				if (bms.bms_acsflag)
					...
					sprintf(tbufv, "solve acs: %ld   mean solve acs: %.2f\n",
						bms.bms_solve_acs,
						((lrT) bms.bms_solve_acs / bms.bms_solve));
					EGPrintTB();
					};
			...
			if (bms.bms_unsol != 0)
				...
				if (bms.bms_acdflag)
					...
					sprintf(tbufv, "unsol acd: %ld   mean unsol acd: %.2f\n",
						bms.bms_unsol_acd,
						((lrT) bms.bms_unsol_acd / bms.bms_unsol));
					EGPrintTB();
					};
				...
				if (bms.bms_acnflag)
					...
					sprintf(tbufv, "unsol acn: %ld   mean unsol acn: %.2f\n",
						bms.bms_unsol_acn,
						((lrT) bms.bms_unsol_acn / bms.bms_unsol));
					EGPrintTB();
					};
				...
				if (bms.bms_acsflag)
					...
					sprintf(tbufv, "unsol acs: %ld   mean unsol acs: %.2f\n",
						bms.bms_unsol_acs,
						((lrT) bms.bms_unsol_acs / bms.bms_unsol));
					EGPrintTB();
					};
